"""
	...
	@description: Model architecture derived from torch
	@author: JungiLee
"""
...
class ResNet(nn.Module):
	...
	def extract_features(self, x, n=0):
	...
	def renew_layers(self, last_num_layers):
		...
		if last_num_layers >= 3:
			...
			self.inplanes = 64	# reset input dimension to 1th block output
			self.layer2 = self._make_layer(block, 128, layers[1], stride=2,
										   dilate=replace_stride_with_dilation[0])
		if last_num_layers >= 2:
			print("re-initalize block 3")
			self.inplanes = 128  # reset input dimension to 2th block output
			self.layer3 = self._make_layer(block, 256, layers[2], stride=2,
									   dilate=replace_stride_with_dilation[1])

		if last_num_layers >= 1:
			print("re-initalize block 4")
			self.inplanes = 256  # reset input dimension to 3th block output
			self.layer4 = self._make_layer(block, 512, layers[3], stride=2,
									   dilate=replace_stride_with_dilation[2])
		print("re-initalize the final layer")
		n_classes= self.fc.out_features
		self.fc= nn.Linear(self.in_layer, n_classes)
...
class PreActResNet(nn.Module):
	...
	def renew_layers(self, last_num_layers):
		...
		print("re-initalize the final layer")
		self.linear = nn.Linear(self.in_layer, self.num_classes)




def PreActResNet18(n_class=10):
	return PreActResNet(PreActBlock, [2, 2, 2, 2], n_class)
#def PreActResNet18(pretrained=False, progress=True, **kwargs):
#	return _resnet('resnet18', PreActBlock, [2, 2, 2, 2], False, progress, **kwargs)
